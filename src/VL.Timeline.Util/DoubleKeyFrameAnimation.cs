...
namespace VL.Timeline.Util
{
    ...
    public class DoubleKeyFrameAnimation : DoubleAnimationBase, IKeyFrameAnimation, IAddChild
    {
            ...
            AnimationClock animationClock)
        {
            ...
            if (IsAdditive)
            {
                return defaultOriginValue + currentIterationValue;
            }
        ...
        {
            Debug.Assert(_areKeyTimesValid,
            ...
            get
            {
                ...
                if (_keyFrames != null)
                {
                    ...
                    for (var index = 0; index < keyFrameCount; index++)
                    {
                        ...
                        if (keyTime.Type != KeyTimeType.TimeSpan) continue;
        ...
        {
            Debug.Assert(!_areKeyTimesValid,
                "KeyFrameDoubleAnimaton.ResolveKeyTimes() shouldn't be called if the key times are already valid.");
            ...
            foreach (var t in unspecifiedBlocks)
            {
                var block = (KeyTimeBlock) t;
                ...
                var uniformTimeStep =
                    TimeSpan.FromTicks((_sortedResolvedKeyFrames[block.EndIndex]._resolvedKeyTime - blockBeginTime)
                                       .Ticks / segmentCount);
        ...
        private void ResolvePacedKeyTimes()
        {
            ...
            do
            {
                if (_keyFrames[index].KeyTime.Type == KeyTimeType.Paced)
                {
                    ...
                    var prevKeyValue = _keyFrames[index - 1].Value;

                    do
                    {
                        var currentKeyValue = _keyFrames[index].Value;

                        // Determine the segment length for this key frame and
                        // add to the total length.
                        totalLength += Math.Abs(currentKeyValue - prevKeyValue);
                    ...
                    totalLength += Math.Abs(_keyFrames[index].Value - prevKeyValue);
                        ...
                        i++, currentKeyFrameIndex++)
                    {
                        ...
                        _sortedResolvedKeyFrames[currentKeyFrameIndex]._resolvedKeyTime =
                            prePacedBlockKeyTime +
                            TimeSpan.FromMilliseconds((segmentLengths[i] / totalLength) *
                                                      pacedBlockDuration.TotalMilliseconds);
                    }
                }
                else
                {
